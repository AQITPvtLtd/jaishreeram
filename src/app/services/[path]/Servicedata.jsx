...
import Head from "next/head"; // ✅ Add Head for meta tags
...
const Servicedata = ({ path }) => {
  ...
  // ✅ Update browser tab title dynamically after data loads
  useEffect(() => {
    if (serviceDetailed) {
      document.title = serviceDetailed.meta_title || serviceDetailed.title;
    }
  }, [serviceDetailed]);

  if (!serviceDetailed) {
    return <p className="text-red-600">Service not found.</p>;
  }
  ...
  return (
    <>
      {/* ✅ SEO Meta Tags */}
      <Head>
        <title>{serviceDetailed.meta_title || serviceDetailed.title}</title>
        <meta
          name="description"
          content={
            serviceDetailed.meta_desc ||
            serviceDetailed.short_desc ||
            ""
          }
        />
        <meta
          name="keywords"
          content={serviceDetailed.meta_keyword || ""}
        />
      </Head>
